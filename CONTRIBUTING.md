...
- [npm trusted publishing](https://docs.npmjs.com/trusted-publishers) configured for `vitest-browser-three`, so the publish job can authenticate via OIDC (`id-token: write`) without a long-lived npm token.
